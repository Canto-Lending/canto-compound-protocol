...
describe('Flywheel', () => {
  ...
  beforeEach(async () => {
    ...
    comptroller = await makeComptroller({ root: root, compValue: 300 });

    cLOW = await makeCToken({comptroller, supportMarket: true, symbol: 'cLOW', underlyingPrice: 1, interestRateModelOpts});
    // cREP = await makeCToken({comptroller, supportMarket: true,  symbol: 'cREP', underlyingPrice: 2, interestRateModelOpts});
    // cZRX = await makeCToken({comptroller, supportMarket: true,  symbol: 'cZRX', underlyingPrice: 3, interestRateModelOpts});
    // cEVIL = await makeCToken({comptroller, supportMarket: false, symbol: 'cEVIL', underlyingPrice: 3, interestRateModelOpts});
  });
  ...
  //   // console.log("comptroller KEYS: ",Object.keys(comptroller.methods));
  //     const tx = await send(comptroller, '_grantComp', [a1, 100]);
  //     // const tx = await comptroller.methods._grantComp(a1, 100);
  //     console.log("TX KEYS: ",Object.keys(tx));
  //     // await expect(tx).toHaveLog('CompGranted', {
  //     //   recipient: a1,
  //     //   amount: 100
  //     // });
  //     await expect(tx).toHaveLog('CompGranted',{ recipient: a1, amount: 100 });
  // });
  ...
  //   ).rejects.toRevert('only admin can grant comp');
  // });
